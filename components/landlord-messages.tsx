  ...
  const loadThreads = async () => {
    ...
    try {
      const response = await fetch(
        `/api/messages?userId=${encodeURIComponent(currentUser.id)}&role=resident`,
        { cache: "no-store" },
      );
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-3">
      <Card className="rounded-2xl lg:col-span-1">
        <CardHeader>
          <CardTitle className="text-base font-semibold">WG Listing Chats</CardTitle>
        </CardHeader>
            ...
            threads.map((thread) => (
              ...
              >
                ...
                <p className="text-xs text-muted-foreground line-clamp-1">
                  {thread.studentName} - {thread.district}
                </p>
      ...
      <Card className="rounded-2xl lg:col-span-2">
        <CardHeader>
          <CardTitle className="text-base font-semibold flex items-center gap-2">
            ...
            {selectedThread ? `${selectedThread.homeTitle} - ${selectedThread.studentName}` : "Select a chat"}
          </CardTitle>
        </CardHeader>
        <CardContent>
          {error ? (
          ...
          {selectedThread ? (
            ...
            <p className="text-sm text-muted-foreground">Pick a chat thread from the left.</p>
          )}
        </CardContent>
      </Card>
    </div>
